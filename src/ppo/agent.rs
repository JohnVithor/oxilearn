...
use crate::{env::Env, optimizer_enum::OptimizerEnum};

use super::{categorical::Categorical, model::Policy, parameters::ParametersPPO, rollout::Rollout};
...
impl PPOAgent {
    pub fn new(
        ...
        let rollout_buffer = Rollout::new(num_steps, obs_size, device);
        PPOAgent {
            optimizer: optimizer
                .build(policy.varstore(), parameters.learning_rate)
                .unwrap(),
            policy,
            rollout_buffer,
            parameters,
    ...
    pub fn collect_rollout(
        ...
        for _p in 0..self.rollout_buffer.capacity {
            let (new_next_obs, new_next_done) = self.collect_step(env, next_obs, next_done);
    ...
    fn collect_step(
        ...
        let (new_obs, reward, terminations, truncations) = env.step(action_v as usize).unwrap();
        let new_next_done = terminations | truncations;
        let new_next_obs = if new_next_done {
            env.reset(None).unwrap().to_device(self.device)
        } else {
    ...
    fn compute_advantages(&mut self, next_obs: &Tensor, next_done: bool) -> Tensor {
        ...
        for t in (0..self.rollout_buffer.step).rev() {
            ...
            let delta = &self.rollout_buffer.rewards.get(id)
                + self.parameters.gamma * nextvalues * nextnonterminal
                - &self.rollout_buffer.values.get(id);
            let advj = delta
                + self.parameters.gamma * self.parameters.gae_lambda * nextnonterminal * lastgaelam;
    ...
    fn flatten_data(
        ...
        let b_obs = self
            .rollout_buffer
            .obs
            .view([-1, self.rollout_buffer.obs_size]);
    ...
    fn optimize_epoch(
        ...
        let b_inds = Tensor::randperm(
            self.parameters.batch_size as i64,
        ...
        for start in (0..self.parameters.batch_size).step_by(self.parameters.minibatch_size) {
            let end = start + self.parameters.minibatch_size;
    ...
    pub fn optimize(&mut self, advantages: Tensor, returns: Tensor) -> OptimizationResults {
        ...
        for _epoch in 0..self.parameters.update_epochs {
            let (
                epoch_old_approx_kl,
                epoch_approx_kl,
                epoch_pg_loss,
            ...
            if let Some(target_kl) = self.parameters.target_kl {
        ...
        OptimizationResults {
            learning_rate: self.parameters.current_lr,
            value_loss: v_loss,
            policy_loss: pg_loss,
    ...
    fn calculate_policy_loss(
        ...
        let pg_loss2 = -mb_advantages
            * ratio.clamp(
                1.0 - self.parameters.clip_coef,
                1.0 + self.parameters.clip_coef,
            );
        let pg_loss = pg_loss1.max_other(&pg_loss2).mean(Kind::Float);

        let newvalue = newvalue.view([-1]);
        let v_loss = if self.parameters.clip_vloss {
            let v_loss_unclipped = (&newvalue - mb_returns).pow(&Tensor::from(2));
            let v_clipped = mb_values
                + (&newvalue - mb_values)
                    .clamp(-self.parameters.clip_coef, self.parameters.clip_coef);
        ...
        let loss: &Tensor = &(pg_loss.shallow_clone() - self.parameters.ent_coef * entropy_loss
            + (v_loss / 2.0) * self.parameters.vf_coef);
    ...
    pub fn learn(
        ...
        let mut next_obs = env.reset(Some(seed)).unwrap().to_device(self.device);
        let mut next_done = false;

        let mut results = vec![];
        let mut checkpoint = num_iterations / 10;

        for global_step in (0..num_iterations).step_by(self.rollout_buffer.capacity as usize) {
            if anneal_lr {
                self.anneal_learning_rate(global_step, num_iterations);
            }
            let (new_next_obs, new_next_done) = self.collect_rollout(env, next_obs, next_done);
            ...
            if global_step >= checkpoint {
                ...
                let evals = self.evaluate(eval_env, 10);
                let mean_epi_return: f32 = evals.iter().sum::<f32>() / evals.len() as f32;
                println!(
                    "step {}/{} mean: {}",
                    global_step, num_iterations, mean_epi_return
                );
                if eval_env.reward_threshold().is_some()
                    && mean_epi_return > eval_env.reward_threshold().unwrap()
                {
    ...
    pub fn evaluate(&mut self, eval_env: &mut impl Env, num_episodes: usize) -> Vec<f32> {
        let mut rewards = vec![];

        for _ in 0..num_episodes {
            let mut obs = eval_env.reset(None).unwrap().to_device(self.device);
            let mut done = false;
            ...
            while !done {
                ...
                let (new_obs, reward, terminations, truncations) =
                    eval_env.step(action.int64_value(&[0]) as usize).unwrap();
